...
CREATE (sally:class:person { key:'person0', name:'Sally' })-[:class_of]->person
CREATE (anne:class:person { key:'person1', name:'Anne' })-[:class_of]->person
CREATE (john:class:person { key:'person2', name:'John' })-[:class_of]->person
CREATE (pam:class:person { key: 'person3', name:'Pam' })-[:class_of]->person
...
MATCH p=p0:class:person-[:event]->ev-[:event]->p1:class:person 
WITH p, ev 
MATCH time_identity = d0:d<-[:event]-ev 
WITH d0, p 
MATCH p1=d0-[:child_of*]->y0:y 
RETURN extract(x IN nodes(p)| coalesce(x.name, x.key)), reduce(s = '' , n IN nodes(p1)| n.tempo + s) AS TimeIdentity 
ORDER BY TimeIdentity
